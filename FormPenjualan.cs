...
namespace KP
{
    public partial class FormPenjualan : Form
    {
        ...
        {
            int index = cbBarang.FindString(cbBarang.Text);
            cbBarang.SelectedIndex = index;
            try
            {
                Koneksi.openConn();
        ...
        private void btnUpdate_Click(object sender, EventArgs e)
        {
            ...
            if (index!=-1)
            {
                ...
                foreach (string a in temp)
                {
                    hargabarang = hargabarang + a;
                }
                temp = tbJumlah.Text.Split('.');
                string jumlahbarang = "";
                foreach (string a in temp)
                {
                    jumlahbarang += a;
                }
                temp = tbTotalBarang.Text.Split('.');
                string totalbarang = "";
                foreach (string a in temp)
                {
                    totalbarang += a;
