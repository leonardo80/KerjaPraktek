...
namespace KP
{
    public partial class FormFindSupplier : Form
    {
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            {
                da = new MySqlDataAdapter("select * from supplier where id_supplier like '%" + search + "%' ", Koneksi.conn);
                ds = new DataSet();
                da.Fill(ds);
            ...
            {
                da = new MySqlDataAdapter("select * from supplier where nama_supplier like '%" + search + "%' ", Koneksi.conn);
                ds = new DataSet();
                da.Fill(ds);
                dataGridView1.DataSource = ds.Tables[0];
            }
            ...
            {
                da = new MySqlDataAdapter("select * from supplier where alamat_supplier like '%" + search + "%' ", Koneksi.conn);
                ds = new DataSet();
                da.Fill(ds);
                dataGridView1.DataSource = ds.Tables[0];
            ...
            {
                da = new MySqlDataAdapter("select * from supplier where kota_supplier like '%" + search + "%' ", Koneksi.conn);
                ds = new DataSet();
                da.Fill(ds);
                dataGridView1.DataSource = ds.Tables[0];
            }
